# EXPAND↔TIGHTEN balance audit — 2026-07-16 (v2, post-amendment tree)

Provenance: multi-agent audit (10 subsystem mappers -> synthesis -> adversarial
verify, 3 lenses per finding: doctrine / money-safety / effectiveness), run
16-Jul on the operator's instruction "ensure the expand vs tighten is balanced
for real money and all other bots too". This v2 numbering supersedes the
morning run's; the synthesis reviewed the SHIPPED balance diff and graded all
eight board/rail fixes SOUND. DB ground truth at audit time: Tide Rider live
0 closes ever / -$0.23 mark; Funding Farmer 14 closes, lifetime -$0.30, 7d
+$2.38.

Verification status: lenses for IMB-01..-10 completed (majority rule);
IMB-11..-32 + the per-fix refuter passes were cut off by the session usage
limit and are queued to finish after the reset — treat their entries as
UNVERIFIED synthesis findings until then.
...
### IMB-02 [medium] — VERIFIED real (one refuter executed the dd_governor code)
...
### IMB-03 [medium] — survives 2/3 (one lens: fail direction arguably safe)
...
### IMB-05 [medium] — REFUTED 2/3 — treat as document-the-exemption, not enforce
...
### IMB-08 [medium] — survives 2/3 (doctrine lens: freeze lands restrict-side — calibration item)
...
### IMB-09 [medium] — REFUTED 2/3 — dead code only, fails safe; housekeeping not risk
...
### IMB-11 [medium] — unverified (queue)
...
### IMB-12 [medium] — unverified (queue)
