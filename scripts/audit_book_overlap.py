...
def living_gates(cur):
    ...
    for bot, extra in cur.fetchall():
        ...
        for key, dest in (("enter_apr", "enter_apr"), ("min_vol", "min_vol"),
                          ...
                          ("apr_hi", "apr_hi"),
                          # [(qk)] the persistence gate — published since the
...
KNOWN_CELL_COLLISIONS = {
    frozenset({"perps-funding-carry-lshadow", "book-kiyosaki-lshadow"}):
        ...
        "exits are its own (liability_flip 6h, decay_paid). [(qk) 18-Aug: the "
...
def supply_in(rows, classes, region, persist_h=6.0):
    ...
    [(qk)] `persist_h` is a CELL parameter, and 6.0 is only the historical
    ...
    🌾 carry gates at 12h since (qk) while 🏦 Rich Dad stays at 6h — so a
    ...
    for grp in found:
        ...
        for b in sorted(grp):
            ...
            print(f"     {b:32s} apr=[{g['enter_apr']:.3g},"
                  ...
                  f"{'inf' if mx is None else f'{mx/1e6:.2f}M'})"
                  # [(qk)] differential REACH on a shared cell: two books at
