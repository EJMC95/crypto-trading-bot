...
[2026-09-02, Eamon: "Start it." — the build (xk) named and did not do.]

===========================================================================
WHY THIS EXISTS, and why it is NOT the cut (xk) registered

`(xk)` proposed excluding mum's non-crypto names, then REFUTED its own
...
REUSE, NEVER RETYPED ((hj) — a second copy of a rule is a second rule):
  ...
  * the class axis: `fleet_bus.is_crypto` — the same owner (xk) graded on
...
def reachability(eps, ladder, stop, hold):
    """Per class: can the SHIPPED rungs actually be reached? The (xk)
