...
QUANTITIES = {
    ...
    # [2026-09-02 (ws)] the breakout arm's OWN clock, split from
    ...
    # [2026-09-02 (ws)] 👩 mum's judge lane. rsi_max cuts the RSI at entry
    # (stamped `rsi_entry` on every close since (ws)); max_hold_min is a
...
def _measure(dsn):                                        # pragma: no cover
    ...
    for name, q in QUANTITIES.items():
        ...
        if q["source"].startswith("state:"):
            ...
            if kind == "hold_h":
                # [(ws)] optional `arg` = a substring of the exit reason that
