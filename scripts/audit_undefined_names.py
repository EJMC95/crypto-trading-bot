...
# Shipped surface: the bots and organs that run in a container.
#
# [2026-08-16] `tests` USED TO BE EXCLUDED, on the reasoning that "a NameError
# there fails a run a human is watching, not a trading loop nobody is". That
# reasoning was falsified by measurement, so the exclusion is gone:
# `tests/test_selftests.py` carried `_tail(om)` where it means `_tail(ok)` —
# a concurrent changelog-letter re-letter, done as a blanket replace over the
# file, rewrote a VARIABLE that happened to look exactly like a citation. The
# name sits inside an f-string that is evaluated ONLY when an audit fails, so
# it was invisible until an audit did fail, and then it did the opposite of
# "failing a watched run": it REPLACED the audit's real message with
# `NameError: name 'om' is not defined`, hiding the diagnosis at the one moment
# it was load-bearing, and left main red on a misleading error. A NameError in
# the harness that reports failures is not a lesser bug than one in a bot — it
# is the bug that hides the others. Scanning tests costs ~0.7s.
SCAN_DIRS = ("", "venues", "parliament", "user_data/strategies", "tests")
...
def _selftest() -> None:
    ...
    # ------------------------------------------------------------------
    # 7. [2026-08-16] THE SCOPE ARM'S OTHER HALF — tests/ is IN, and the
    #    incident that put it there. This file used to skip `tests` because
    #    "a NameError there fails a run a human is watching, not a trading
    #    loop nobody is". Measured counterexample: a blanket changelog-letter
    #    replace rewrote `_tail(ok)` to `_tail(om)` in tests/test_selftests.py
    #    — inside an f-string evaluated ONLY when an audit fails. So it did
    #    not fail a watched run; it REPLACED the audit's message with
    #    `NameError: name 'om' is not defined`, hid the diagnosis at the one
    #    moment it mattered, and left main red on a misleading error. The
    #    harness that reports failures is not lower-stakes than a bot: it is
    #    the thing that tells you a bot is broken.
    # ------------------------------------------------------------------
    assert "tests" in SCAN_DIRS, (
        "tests/ dropped out of scope — the 16-Aug NameError that hid an "
        "audit's own failure message lived there")
    assert "tests" not in SKIP_DIRS, "tests/ is skipped again"
    harness = (
        "def _tail(x):\n"
        "    return x\n"
        "def test_thing(ok):\n"
        '    assert ok.returncode == 0, f"failed:\\n{_tail(om)}"\n')
    found, _ = check_source(harness, "tests/test_selftests.py")
    assert any("'om'" in f for f in found), (
        f"the exact 16-Aug damage is not caught: {found}")
    assert check_source(harness.replace("_tail(om)", "_tail(ok)"),
                        "tests/test_selftests.py")[0] == [], "false positive on the fix"
    ...
    print("audit_undefined_names --selftest OK (fires on (ib) and on (ig)'s "
          ...
          "idioms incl. every comprehension form, star-import declared, and "
          "tests/ is in scope with the 16-Aug f-string damage pinned)")
