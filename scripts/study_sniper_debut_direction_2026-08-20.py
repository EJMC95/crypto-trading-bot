...
which DIRECTION?  [2026-08-20 (se)]

`(sd)` established what the book CANNOT do: its exit bracket is a near-dead
...
This study asks the question `(sd)` deliberately left open: **if the young
...
DECLARED LIMITS, none fixable from this endpoint:
  ...
  * 1h resolution. The bot polls at 60s, so intra-hour paths are invisible;
    per `(sd)` the exit-basis gap this creates is real and one-sided, which is
...
# The (sd) harness is the ONE owner of walk_exit / slip_bps / score / the
