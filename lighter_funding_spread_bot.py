...
def main():
    ...
    if ctx.mode == "lighter_live":
        ...
        # "perps-funding-spread-lshadow" and NO bare "perps-funding-spread", so
        # the lookup could never match. Fail-closed, so no unsafe admit ever —
        # but I10's gate was structurally UNPASSABLE: a genuinely READY book
        # with FUNDSPREAD_GOLIVE=1 would still be refused, with a message that
        # reads like the grader not knowing the book exists.
        #
        # Fourth instance of the bare-vs-suffixed key class in this fleet
        # (FEE_RT, ERA_START, era_epoch_for). The exact bot_id is deliberate,
        # not a match on either spelling: the suffix is the difference between
        # two books, so a READY SHADOW twin must never arm the LIVE arm.
...
def _selftest():
    ...
    # payload its publisher would actually build, or the test certifies the bug.
    _ROW = BOT + "-lshadow"

    def _payload(ready, updated=None, bars=None):
        return {"updated": updated or _fresh,
                "books": {_ROW: {"ready": ready, "bars_passed": 6 if ready else 3,
    ...
    try:
        ...
        assert golive_blocker(_ROW, _payload(True), _now), "no opt-in must refuse"
        for _v in ("", "0", "true", "yes", "on", "TRUE"):
            os.environ["FUNDSPREAD_GOLIVE"] = _v
            assert golive_blocker(_ROW, _payload(True), _now), \
        ...
        _not_ready = golive_blocker(_ROW, _payload(False), _now)
        ...
        _msg = golive_blocker(_ROW, _payload(False, bars=_bars), _now)
        assert "window" in _msg and "t" in _msg, _msg

        # FAIL-CLOSED on every flavour of missing evidence
        assert golive_blocker(_ROW, {}, _now), "dark gate must refuse"
        assert golive_blocker(_ROW, None, _now), "None gate must refuse"
        assert golive_blocker(_ROW, {"books": {}}, _now), "no stamp must refuse"
        assert golive_blocker(_ROW, _payload(True, updated="not-a-date"), _now), \
            "unparseable stamp must refuse, not pass"
        _st = golive_blocker(_ROW, _payload(True, updated=_stale), _now)
