...
[2026-08-20 (ty)] `(lk)` put ONE instrument-class screen on the sniper's surge
...
CASHCAT Aug). The "86 days dry" figure in `(qi)`/`(tx)` is true of class 2 and
...
1. The two sources ask DIFFERENT questions, and neither silently becomes the
   other. Surge stays crypto-only — its evidence is untouched and `(tx)`
...
def test_the_two_sources_ask_different_questions():
    ...
    # whose surge (tx) measured at -0.840%/trade
...
def test_the_census_shows_where_the_young_source_now_dies():
    """The (tx) funnel and this fix are one mechanism: the census must move.
