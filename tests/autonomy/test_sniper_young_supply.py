...
[2026-08-20 (sl)] `(lk)` put ONE instrument-class screen on the sniper's surge
...
CASHCAT Aug). The "86 days dry" figure in `(qi)`/`(sk)` is true of class 2 and
...
1. The two sources ask DIFFERENT questions, and neither silently becomes the
   other. Surge stays crypto-only — its evidence is untouched and `(sk)`
...
def test_the_two_sources_ask_different_questions():
    ...
    # whose surge (sk) measured at -0.840%/trade
...
def test_the_census_shows_where_the_young_source_now_dies():
    """The (sk) funnel and this fix are one mechanism: the census must move.
