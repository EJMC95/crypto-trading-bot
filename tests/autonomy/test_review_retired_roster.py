...
@pytest.mark.parametrize("alive", [
    # [2026-08-19] freqtrade-mum-lshadow left when v1 was RETIRED (I17
    # no_rate) and [2026-09-09 (zo)] returns as v2's living twin, the day
    # freqtrade-georgia-lshadow left for the same reason (her cap-5 read
    # failed). Two living subjects still exercise the trap; the semantics
    # being guarded did not move.
    "freqtrade-mum-lshadow",         # mum v2's control arm (bare name retired)
...
def test_the_bare_name_trap_does_not_retire_a_living_book(review, alive, legacy):
    ...
    v2, trading now. Membership must stay EXACT-match — a rewrite to
    substring/prefix matching would silently retire healthy books, one of
