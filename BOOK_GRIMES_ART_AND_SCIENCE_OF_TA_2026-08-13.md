...
| 4 | Match the trade to the market regime. | The gate IS the regime switch, mechanically: when the tape changes, a setup's trailing record opens or closes it — no human in the loop, no hand-flipped env. | **Measured at birth, corrected in place per I12 ((mh))**: under the honest LAG-1 trend convention (day D gated on D−1's close — the first cut's unlagged map was a look-ahead in the replay AND a missing key live), keltner reads n=109, +$31.69, **t=0.49 — knife-edge BELOW the 0.5 bar → closed**; pullback −$36.25 → closed; failtest −$12.05 → closed. **The book is born trading NOTHING, by its own rule** — nothing currently tests, the public scorecard says so, and the gate re-decides every 6h. The first cut's "keltner OPEN at t=0.75" was partly the look-ahead's flattery; the correction is the book's own thesis enforcing itself on its author. **[(ml)] The grader gained two more fidelity corrections after birth** — no-claim trend days fail CLOSED at the `trend_at` contract (0 used to PASS keltner's both-ways test, so a failed daily fetch unfiltered the fade inside the replay itself), and the entry bar's own post-open range is bracket-tested (entry-bar stops the live loop realises were invisible to the trailing record — optimism in exactly the numbers that hold a gate open). The container's FIRST live retest (13-Aug 23:51Z, pre-(ml) code): pullback n=113 +$20.22 t=0.29 · failtest n=282 −$103.18 t=−1.71 · keltner n=98 −$3.18 t=−0.05 — all closed, the birth state confirmed on live tape. The published scorecard is the authority; these doc numbers are provenance. |
