...
(The twin of `downtrend_ensemble_bot/fleet_publish.py`. Deliberately COPIED
rather than shared: the two packages are separable by design, and a shared
module between them would make each depend on the other's checkout. The
duplication is 100 lines and the coupling it avoids is the whole point.)
...
ROW_IDS = {
    "adaptive-ensemble": "adaptive-ensemble-lshadow",
}
...
def enabled() -> bool:
    ...
    if os.environ.get("ADAPTIVE_PUBLISH", "").strip().lower() in ("0", "off",
                                                                  "false"):
        return False
