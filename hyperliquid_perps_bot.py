...
def main():
    ...
    def _flatten_live(reason):
        ...
        [2026-07-11] Trail Blazer's daily-loss flatten left NO ledger rows
        (forced closes bypassed venue_orders) — undiagnosable. Closes only
        the coins the scan shows held (each market_close costs a weighted
        venue call — closing all COINS blind can exhaust the tx budget
        mid-crisis); if the scan FAILS, close everything blind (capital
        first) and write a marker row saying so. Fill px / realized P&L are
        NOT captured — market_close is fire-and-forget; real-fill capture is
        the fill-forensics follow-up."""
        if dry_run:   # self-guard: never touch the venue from a paper loop
            return
        try:
            _fpos = ctx.venue.positions()
            _scan_ok = True
        ...
        if halted_today:
            ...
            try:
                _hb_open = (broker.open_count() if dry_run
                            else sum(1 for v in ctx.venue.positions().values()
                                     if v.get("size")))
            except Exception:
