...
### 2026-08-20 (ty) · operator: *"bring the sniper back to life and make it do its intended job"* / *"don't continue if it's going to be negative, restrictive and choking of the bot"*

---

## THE HEADLINE

`(qi)` and `(tx)` both reported the sniper's debut supply as **dead — "ZERO
...
(qi) wait-then-short +6.23%/episode; (tx) supply census −8.30% at 168h; this
...
nothing, and slightly less than nothing. This is `(tx)`'s finding — the entry
...
The surge source is **untouched** — its evidence is real and `(tx)` measured
...
- 1h resolution against a 60s decision loop; per `(tx)` that gap is real and
...
(registered `--selftest`; reuses the `(tx)` harness's exit walker, slippage
