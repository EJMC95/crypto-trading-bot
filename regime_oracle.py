...
def _selftest():
    ...
    with mock.patch(__name__ + "._lget",
            ...
            mock.patch(__name__ + ".time", mock.Mock(time=lambda: now,
                                                     sleep=lambda s: None)):
        # [2026-08-07 (lj)] `sys.exit(store.organ_main(...))` — the production
        # __main__ line, copy-pasted into the selftest. It raised SystemExit(0)
        # and KILLED THE PROCESS HERE, so the 37 assertions below never ran and
        # the final "selftest OK" line never printed, while
        # `tests/test_selftests.py` (which checks the exit code) marked the
        # module PASSING. Every coverage, staleness, dark-venue and per-asset
        # contract in this file was dead. Run the organ, keep the code, assert.
