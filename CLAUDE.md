...
- **A BOT ID IS NOT A LEVER NAMESPACE — AND TWO CORRECT FAIL-OPENS COMPOSE
  INTO SOMETHING NOBODY CAN SEE (4-Sep (yb)).** 👩 mum's shadow arm — the 🧪
  judge's experiment arm, i.e. the fleet's ONLY path from shadow evidence to
  real money — asked `fleet_tuning` for **`xp.mum-lshadow.rsi_max`**, because
  the host built its prefix as `f"xp.{bot_id.split('-', 1)[-1]}."`: it strips
  the FIRST id segment and keeps the venue SUFFIX. The judge writes
  **`xp.mum.`**, which `fleet_bus.JUDGED_PAIRS[...]["xp_prefix"]` had declared
  all along. Measured on the live bus: the lever sat open **36h** while every
  close stamped the env default and the judge held on `arm_skew` — **not one
  experiment had ever applied on that lane.** Same family as `era_epoch_for`'s
  double-`rsplit` ((hd)/(hg)): where a name maps to another name, the mapping
  is DECLARED and READ, never derived by string surgery at the call site.
  **The half worth generalising is why it was invisible for 36h: both
  behaviours in the chain are CORRECT.** `apply_book_levers` degrades to the
  env default on a dark rail — right. `get_lever` returns the caller's default
  for an unregistered name — right. Composed, they make "the judge set
  nothing" and "this arm is asking for a name no registry holds"
  **byte-identical**, which is I18 arriving through a door no single component
  is guilty of. So when two fail-opens meet, **publish the resolution** —
  `lever_surface` names the namespace and whether it exists — because neither
  component can be made to fail without breaking the property that makes it
  correct. The judge's own fail-CLOSED receipt rule (`ran_candidate`) is what
  stopped this reaching a promotion, and it is the reason the cost was a lost
  window rather than real money.
  ENFORCED BY: `tests/autonomy/test_mum_virtual_flip.py::test_every_judged_pair_resolves_to_registered_levers`,
  `tests/autonomy/test_mum_virtual_flip.py::test_the_prefix_is_never_built_from_the_bot_id_again`,
  `experiment_judge.py::VOID_SKEW_H`
