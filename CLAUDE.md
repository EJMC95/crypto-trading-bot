...
**2-Sep (ws)**: THE ORGAN-REVIEW DOCKET CALLS, made in one act (Eamon:
*"Proceed with everything in the organ review"*) — five rows, each on a
MEASURED exclusion or a zero rate: `band-garrett-lshadow` (n=85, t=−2.22, ub
−0.455%), `book-douglas-lshadow` (n=81, t=−2.54, ub −0.357%),
`book-grimes-lshadow` (0 closes in 19d), `freqtrade-georgia-lshadow`
(undecidable, ~4,224 days), `perps-funding-lighter-lshadow` (n=200, t=−2.32,
ub −0.233%; the 🧪 judge's lane moved to 👩 mum the same commit). Both halves
for all five; reversible per book; pinned by
`tests/autonomy/test_docket_retirements_ws.py`. KEPT as recommended: 🧭
nav-cook (marginal exclusion, half its sample pre-fix) and ⚖️ Counterweight
(re-read 1-Oct).
...
- `fleet-tuning` — the growth rail's lever payload (authors: evidence board
  ...
  **[2-Sep (ws)] THE JUDGE'S LANE IS 👩 MUM'S** — `xp.mum.rsi_max` /
