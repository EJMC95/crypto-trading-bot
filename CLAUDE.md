...
- `pnl-dashboard` — STRUCTURAL mention-count, not rework (the `freqtrade-bots` shape at the fleet's ONE READER). 6 entries in 7d ((xj),(wt),(wn),(wl),(vz),(vw)) because this single service serves `/pnl.json`, `/bus.json`, `/watchdog.json` and the 🚦 card, so EVERY change that reads the feed, adds a deploy route or touches observability names it whether or not it changes a line of its code. **Measured, not asserted:** `(xj)` touched **zero** dashboard files (it reads the feed the dashboard serves) and `(wt)` touched **zero**, while `(wn)` touched two (the watchdog's pager thresholds); the genuine dashboard-code changes in the window are `(wl)` (the capital-move-immune day delta) and `(wn)` (the pager), and they are DIFFERENT mechanisms, neither rebuilt twice. Narrowing the extractor to stop counting mentions is REFUSED for the reason the `freqtrade-bots` line already gives: it would blind the guard to a real recurrence in the fleet's only reader, which is exactly where the `(ml)` stale-serving class lives. **Owner: this repo — and if this service recurs on ONE mechanism twice, that is a new house and this line must not be stretched to cover it.**
<!-- RECURRING:END -->
