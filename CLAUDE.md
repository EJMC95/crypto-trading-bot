...
**[2-Sep, edge-audit follow-up — CALIBRATED, then CALIBRATED OPTIMALLY; corrected in place per
I12. Eamon: *"Proceed on all"* → *"Calibrate accordingly"* → *"Calibrate optimally with
findings"*.]** `fleet_proprioception.grade_live` now judges an episode against the TWIN
(REQUIRED — no control arm ⇒ `recorded`, reason `no-control-arm`) and against the book's ERA
MEAN EXCLUDING THE MOTIVATING WINDOW (this rule's own counterfactual), and **every margin is
DERIVED: the comparison's own standard error at the fleet's critical value**
(`fleet_allocation.t_crit`, one owner), floored at 0.25pp (`LIVE_MARGIN_PP`) against a
zero-variance history; `LIVE_BASE_MIN_N` is pinned to `fleet_allocation.MIN_N` (10). The path
here is the record of why. The first fix gave the pre-window a FIXED margin equal to the mean
reversion — 1.7pp, re-measured on 3,801 closes / 26 books at **+1.74 / +1.52 / +1.60 / +1.67pp
for K = 10 / 15 / 20 / 30** (SE ≈ 0.5), a collapse that does NOT shrink with K because the
books' means drift. But a margin AT the mean is the wrong object: on the no-change control it
still called a neutral change `bad` **21.3%** of the time after a hot motivating window and
`good` **19.2%** after a cold one, and 1.7pp was 2.9 SE on mum's live book while 0.7 SE on a
volatile one — one constant, a different false rate on every book. Derived margins against the
book's mean read **16.0% / 9.9%** on the same control (nominal 10%; the residual is the drift
itself, which only the twin removes — so the twin decides and the book corroborates), with
power on a planted −2pp harm of 74% (was 66%). The instrument
`scripts/study_do_our_changes_hurt_2026-08-27.py --margin --ledger` runs the SHIPPED
`book_gate` on the control and grades its false rates (INSIDE / DRIFT / THIN, exit 2 on
DRIFT). What a lever's consumers do with a verdict is untouched.
...
against the right baseline is the fleet's only way to compound.
  ENFORCED BY: `fleet_proprioception.py::grade_live`, `scripts/study_do_our_changes_hurt_2026-08-27.py::peak_arm`, `scripts/study_do_our_changes_hurt_2026-08-27.py::K`, `fleet_proprioception.py::judge_windows`, `fleet_proprioception.py::book_baseline`, `scripts/study_do_our_changes_hurt_2026-08-27.py::margin_arm`, `tests/autonomy/test_edge_audit_followups.py::test_i25_margins_are_the_books_own_noise_at_the_fleets_critical_value_and_the_twin_is_required`
...
- `golive-readiness` — STRUCTURAL mention-count, not rework (the `freqtrade-bots` shape at the fleet's ONE GRADING AUTHORITY). It is the single owner of the era, the six bars, the drawdown convention and the horizon, so **every instrument that measures anything must import it and calibrate against it**, and therefore names it. The 6 entries in 7d are six different mechanisms, none of which rebuilds the grader: `(yp)` a position-sizing study importing `GOLIVE_MAX_DD` and `stats` as its calibration authority; `(yn)` the lens-veto split published beside `class_split`; `(yg)` the judge's lever prefix, which reads `ready` for the tuner freeze; `(wq)` the claims backlog; `(wp)` the organ board's liveness sweep; `(wo)` the edge audit. Being the thing every measurement must agree with is the opposite of a house being re-entered — it is why those six defects were findable at all, and narrowing the extractor to hide it would blind the guard to a real recurrence in the grader itself. **Owner: this repo — and if this key recurs on ONE mechanism twice, that is a new house and this line must not be stretched to cover it.**
...
registration (the I21/I25 shape); decided AT the read — see 9-Sep (zo) below. Both halves for all
...
**9-Sep (zo)**: `freqtrade-georgia-lshadow` (🔮 georgia v1's SHADOW — the
slate's deferral, resolved at the read it was deferred TO; Eamon: *"take
georgia's read now"*, a day early). The (vb) prediction — cap 3→5 takes
days-to-gate 344→187 (±60) at a HIGHER mean — was graded on the ONLY honest
basis, the 75 closes whose OWN policy stamp reads `max_entries_per_hour: 5`
(keyed on the OPEN, (hc)): **mean −0.0025%/trade vs +0.108% predicted, t
−0.02 vs +2.54, 5.72 closes/day vs 5.47** — the throughput limb delivered, the
mean limb did not, so the t bar is unreachable where 187 days was promised.
**Retired on I17's UNDECIDABLE call** (the organ: era n=277, t=0.50,
`n_req_t` 8,094 closes at 5.12/day ≈ 4.3 years) and **NOT as a measured
loser**: her post-cap upper bound is +0.196% > 0 and this entry says so, so the
precedent cannot be mis-cited. THE TRAP the read stepped around: the claim's
owner field, `horizon.eta_days`, is `null` on an `undecidable` book, so as
registered it could only ever read UNRESOLVED — the ledger gained a terminal
**GRADED** state and the row STAYS in the table with its read recorded.
Instrument: `scripts/study_georgia_cap5_read_2026-09-09.py` (calibration gate
REFUSES on a dark feed or a wrong basis; reproduced the grader's era mean to
0.001pp). Zero open positions, so nothing froze; v3
(`freqtrade-georgia-v3-lshadow`) is a different strategy and stays. Both
halves; `lighter_family_bot.RETIRED_BOOKS` key `freqtrade-georgia`
(exact-match — v3 shares the prefix); reversible via
`GEORGIA_RETIRED_OVERRIDE=run`; pinned by `tests/autonomy/test_september_slate.py`.
...
- `fleet-tuning` — the growth rail's lever payload (authors: evidence board
  ...
  **[6-Sep (ye)] AND FOR FOUR DAYS HER ARM COULD NOT HEAR IT — BY ONE STRING.**
  The shadow host rebuilt the lever prefix from its suffixed row id
  (`f"xp.{bot_id.split('-', 1)[-1]}."` → `xp.mum-lshadow.`, and
  `xp.avo-maria-lshadow.` for 🙏 avo), names the registry does not hold, so
  `get_lever` returned the default at its unregistered rung and two consecutive
  candidates voided (`mum-rsi-32` VOIDED-NEVER-APPLIED at 37.7h; `mum-vel-12-20`
  ran 34h with the arm publishing `vel_band [-999, 999]`, `vel_blocked 0`). The
  prefix is DECLARED (`fleet_bus.JUDGED_PAIRS[*]["xp_prefix"]`) and read through
  `fleet_bus.xp_prefix_for` — never rebuilt — and the class is closed by AST:
  every name a call site can construct must resolve in the registry, and the
  live host's `_BOOKS` token must agree with the pair's. The existing test had
  called `apply_book_levers(s, "xp.mum.")` with the correct prefix HARD-CODED
  — it exercised the function and never the call site.
  ENFORCED BY: `fleet_bus.py::xp_prefix_for`,
  `tests/autonomy/test_judge_lever_prefix_reaches_the_arm.py::test_the_call_site_reads_the_declaration_and_never_builds_the_string`
...
- **THE 30-DAY GO-LIVE CLOCK RESTARTS ON EVERY POLICY CHANGE (30-Jul (hm)).**
  ...
  **[6-Sep (ye)] "FREEZE ITS BARS FIRST" HAS AN ACTUATOR NOW — A READY BOOK
  KEEPS THE BRACKET IT PASSED ON.** (hm) and (jf) were both right and together
  left a hole: the era signature deliberately excludes bracket levers (or the
  30-day bar is unreachable), so nothing stopped the tuner moving a READY
  book's bracket. Measured 6-Sep on the fleet's FIRST-EVER READY verdict — 🎫
  the taker, 6 of 6, t=2.60 — the graded sample spans `taker.tp` 0.03/0.04 and
  `taker.max_hold_h` 24/48 (3 tp episodes, 5 momo episodes on proprioception).
  ...
  ENFORCED BY: `lighter_scout_tuner.py::apply_ready_freeze`,
  `lighter_scout_tuner.py::FROZEN_WHEN_READY`,
  `tests/autonomy/test_tuner_ready_freeze.py::test_the_freeze_runs_at_the_write_site_after_every_other_filter`
