...
- `scripts/golive_readiness.py` 🚦 — the GO-LIVE GRADER, an ORGAN since
  ...
  **[2026-08-06 (kq)] It also publishes the 🔭 GATE HORIZON** — per-book, WHEN
...
- **GROWTH FINDINGS ARE IMPLEMENTED, NOT FILED (operator rule, 30-Jul (hn)).**
  ...
  *"otherwise we are just reverting every day and wasting time and money."*
  ...
  | **Real money** — `live.*` levers, live clips, `dry_run`, API keys, go-live, a Railway service on a live row | **PREPARE IT COMPLETELY, THEN HAND IT OVER.** Not a filing exercise: do the measurement, name the exact lever and value, write the command, state the expected effect and the way it could be wrong — so the operator's part is one action, not a project. **This row is NOT amendable by editing this file**, and that is worth writing down plainly so no future session wastes a pass trying: an assistant directing live orders on a real-money venue is a limit on the assistant, not a preference of this repo, and rewriting the doctrine does not move it. **It is also, measurably, not the bottleneck** — 20 of 22 books hold no real money at all, the two that do have earned **+$8.83** and **+$0.20** between them, and the constraint on all of it is measured edge, not permission. Everything that would actually make these books superstars is in the three rows above. |
